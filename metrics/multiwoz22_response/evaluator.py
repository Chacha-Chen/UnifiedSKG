...
class EvaluateTool(object):
    ...
    def evaluate(self, preds, golds, section):
        ...
        for gold, pred in zip(golds, preds):
            if gold['dialogue_id'].split('.')[0].lower() not in construct_input.keys():
                construct_input[gold['dialogue_id'].split('.')[0].lower()] = []
            current_turn = {}
            current_turn['response']  = pred
            construct_input[gold['dialogue_id'].split('.')[0].lower()].append(current_turn)
            # construct_input[gold['dialogue_id']]['response'] = pred

        e = Evaluator(bleu=True, success=True, richness=True)  ##TODO

        results = e.evaluate(construct_input)
        print(f"BLEU: {results}")
        summary = {}
        summary.update({"Joint Acc": results['bleu']['mwz22']})
...
if __name__ == '__main__':
    import json
    import os
    dir = '/home/ubuntu/chacha_code/UnifiedSKG/output/T5_base_prefix_multiwoz22_response_context/'

    with open(os.path.join(dir, "predictions_predict.json")) as f:
